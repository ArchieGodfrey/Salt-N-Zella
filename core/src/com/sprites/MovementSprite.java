...
public class MovementSprite extends SimpleSprite {
    ...
    private void accelerate() {
        ...
        int vertices = verticesOnRoad(this.collisionLayer);
        // If the sprite should collide, check that all 3 vertices are on a road, then move the sprite
        if (this.collisionLayer == null || vertices == getPolygonVertices(super.getMovementHitBox()).size) {
     ...
     */
    /**
     * Checks if the tile at a location is a "road" tile or not.
     * @return Whether the sprite is on a road (true) or not (false)
     */
    private int verticesOnRoad(TiledMapTileLayer layer) {
        int vertices = 0;
        if (layer != null) {
